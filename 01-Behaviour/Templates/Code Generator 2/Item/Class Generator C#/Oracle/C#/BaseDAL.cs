...
namespace #NAMESPACEDAL#
{
    public abstract class BaseDAL<T>
    {
        ...
        public abstract Int32 Insert(T entity, OracleTransaction oSqlTransaction);
        public abstract void Update(T entiy);
        public abstract void Update(T entity, OracleTransaction oSqlTransaction);
        public abstract void Delete(Int32 key);
        public abstract void Delete(Int32 key, OracleTransaction oSqlTransaction);

        protected List<T> ReturnList(string procedure, ArrayList parameters)
        {
            List<T> returnlist = new List<T>();

            using (OracleDataReader dataReader = SQLHelper.ExecuteReader(procedure, parameters))
            {
        ...
        protected List<T> ReturnList(string procedure)
        {
            ...
            using (OracleDataReader dataReader = SQLHelper.ExecuteReader(procedure))
            {
                if (dataReader.HasRows)
                {
                    returnlist = new List<T>();
        ...
        protected T ReturnUnique(String commandText, ArrayList parameters)
        {
            ...
            using (OracleDataReader dataReader = SQLHelper.ExecuteReader(commandText, parameters))
            {
                if (dataReader.HasRows)
                {
                    dataReader.Read();
        ...
        protected T ReturnUnique(string commandText)
        {
            ...
            using (OracleDataReader dataReader = SQLHelper.ExecuteReader(commandText))
            {
                if (dataReader.HasRows)
                {
                    dataReader.Read();
        ...
        public T ConvertObject(OracleDataReader dr)
        {
		DataRow row;
