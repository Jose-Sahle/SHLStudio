...
void CUGCtrlEx::SetResultTable(SHLOleDBTable* table, TCHAR* szQuery)
{
	...
	try
	{
		...
		{			
			...
			for(INT nColIndex = 0; nColIndex < table->ColumnCount; nColIndex++)
			{
				CString strValue = table->GetValueToString(/*nRowIndex*/0, nColIndex, _T("<null>"));
				if(strValue.Find(_T("SHLStudio Error:")) == 0)
				{
					bSHLStudioError = TRUE;
					strValue.Replace(_T("SHLStudio Error:"), _T(""));
				}
		...
		RedrawAll();
		BestFit(-1, GetNumberCols()-1, 0, UG_BESTFIT_TOPHEADINGS | UG_BESTFIT_AVERAGE);
	}
	catch(CException* ex)
	{
