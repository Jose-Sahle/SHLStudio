...
void CUGCtrlEx::ShowRowHeader(BOOL show)
{
	if(show)
		SetColWidth(-1, m_colheadersize);
	else
	{
		m_colheadersize = GetColWidth(-1);
		SetColWidth(-1, 0);
	}
}

void CUGCtrlEx::ShowColHeader(BOOL show)
{
	if(show)
		SetRowHeight(-1, m_rowheadersize);
	else
	{
		m_rowheadersize = GetRowHeight(-1);
		SetRowHeight(-1, 0);
	}
}

void CUGCtrlEx::AddColumn(CString columnName, INT columnSize, CUGCell *cell, BOOL canEdit)
{
	INT nCols, nColIndex;
	...
	try
	{
		...
		m_cellType.AddItem(cell);
		m_canEdit[nColIndex] = canEdit;

		//	Project - 250
		QuickSetText(nColIndex, -1, columnName);
		QuickSetTextColor(nColIndex, -1, textColorHeaders);
		QuickSetBackColor(nColIndex, -1, backColorHeaders);
		SetColWidth(nColIndex, columnSize);		
...
void CUGCtrlEx::SetResultTable(SHLOleDBTable& table, INT initRow)
{
	INT nRows;
	...
	try
	{
		...
		{			
			...
			QuickSetTextColor(-1, nRowIndex+initRow, textColorHeaders);
			QuickSetBackColor(-1, nRowIndex+initRow, backColorHeaders);

			for(INT nColIndex = 0; nColIndex < GetNumberCols(); nColIndex++)
			{
				DoEvents();

				cell = m_cellType.GetItemAt(nColIndex);
				SetCell(nColIndex, nRowIndex, cell);
				RedrawCell(nColIndex, nRowIndex);

				strColumnName = QuickGetText(nColIndex, -1);
				strValue = table.GetValueToString(nRowIndex, table.GetColumnOrdinal(strColumnName), _T(""));
				QuickSetText(nColIndex, nRowIndex+initRow, strValue);
				if(nRowIndex % 2 == 0)
				{
